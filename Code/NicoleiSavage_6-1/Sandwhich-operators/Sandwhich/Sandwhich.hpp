...
/*
 ...
 For clarity, different breads cost a different amount, likewise meats and cheeses each can cost a different amount. Create as many additional ructors as makes sense. Create whatever methods you think are appropriate.
 */
...
public:
    ...
    string to_string();
    friend ostream& operator<<(ostream& os, Sandwhich& s);
    friend istream& operator>>(istream& is, Sandwhich& s);
    bool operator==(Sandwhich& s);
    bool operator!=(Sandwhich& s);
    bool operator<(Sandwhich& s);
    bool operator>(Sandwhich& s);
    bool operator<=(Sandwhich& s);
    bool operator>=(Sandwhich& s);
    Sandwhich operator++();
    Sandwhich operator++(int);
    Sandwhich operator-(Sandwhich& s);
    Sandwhich operator+(Sandwhich& s);
    Sandwhich operator*(Sandwhich& s);
    Sandwhich operator/(Sandwhich& s);
    
    void sandCompare(Sandwhich& s1, Sandwhich& s2);
    void sandOperators(Sandwhich& s1, Sandwhich& s2);    
};
