...
public:
    ...
    Type getKthElement(int k) const;
...
template <class Type>
Type linkedListType<Type>::getKthElement(int k) const
{
    if (isEmptyList())
    {cout << "Error: List is empty." << endl;exit(-1);}

    if (k < 1 || k > count){cout << "Error: Invalid position." << endl;exit(-1);}
...
{
    if (isEmptyList()){cout << "Error: List is empty." << endl;exit(-1);}
    if (k < 1 || k > count){cout << "Error: Invalid position." << endl;exit(-1);}
    if (k == 1){
        ...
        //trailCurrent skips the current node and directly points to the next node in the list.
        trailCurrent->link = current->link;
        
        //delete current and set last to trail pointer.
