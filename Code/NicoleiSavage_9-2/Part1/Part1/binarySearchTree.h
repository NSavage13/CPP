...
{
public:
    int search (const string& searchItem) const;
...
template <class elemType>
int bSearchTreeType<elemType>::search(const string& searchItem) const
{
    ...
    else
    {
        ...
        {
            if (current->info.key == searchItem)
            {
                found = true;
            }
            else if (current->info.key > searchItem)
            {
                current = current->llink;
                position = position * 2 + 1; // Adjust position for left child
            }
            else
            {
                current = current->rlink;
                position = position * 2 + 2; // Adjust position for right child
            }
        }
    }
